...
get_ck_d_pol_at(a, bi, y_state) = get_ck_d_pol(a)[bi, y_state...]

set_d_pol_at!(a, bi, y_state,  val) = (get_d_pol(a)[bi, y_state...] = val)
set_ck_d_pol_at!(a, bi, y_state, val) = (get_ck_d_pol(a)[bi, y_state...] = val)
...
_find_bond_return_at(::AbstractFixedRateBond, a, bi, biprime, (yi,))  = find_bond_return(a; q = get_q(a)[biprime, yi])

_find_bond_return_at(::AbstractFloatingRateBond, a, bi, biprime, (yi, yi0)) = find_bond_return(a; q = get_q(a)[biprime, yi], κ = get_κ(a)[bi, yi0])


get_c_at(a, bi, bi_prime, y_state) = _get_c_at(get_bond(a), a, bi, bi_prime, y_state)

function _get_c_at(::AbstractFixedRateBond, a, bi, bi_prime, (yi, ))
    q = get_q(a)
    b_grid = get_b_grid(a)
    y_grid = get_y_grid(a)
...
function _get_c_at(::AbstractFloatingRateBond, a, bi, bi_prime, (yi, yi0))
    b_grid = get_b_grid(a)
    y_grid = get_y_grid(a)
    q = get_q(a)
    κ = get_κ(a)
    get_c(a; y = y_grid[yi], b = b_grid[bi], κ = κ[bi, yi0], q = q[bi_prime, yi], b_prime = b_grid[bi_prime])
end 
...
function optimize!(model, a, yi, bi, b_range)
    ...
    default_threshold!(has_runs(model), model, a; bi, yi)
    integrate_v_and_bond_return!(has_runs(model), model, a; bi, yi) 
end
...
function default_threshold!(::WithRuns, model, a; bi, yi)
    ...
    default_threshold!(NoRuns(), model, a; bi, yi) # computing the "standard" default threshold 
    
    # computing the ck default threshold 
    cD = get_c_at(a, bi, biprime, yi)
    eV = get_Ev_at(a, biprime, yi)

    if cD - m_min <= 0 || u(cD - m_min) + β * eV <= vD_val 
        mD_ck = m_min
    else
        mD_ck = clamp(cD - inv_u(u, vD_val - β * eV), m_min, m_max)
    end 
    set_ck_d_pol_at!(a, bi, yi, mD_ck)
end
        ...
        if (mD <= m_min)  
            ...
            for cur in pol
                ...
                elseif m_CK <= m_lag 
                    a1, a2 = integrate_v_helper(model, a, bi, biprime, yi, m_lag, m_k)
                    v_m += (1 - η) * a1
                    bond_return_val += (1 - η) * a2
                else 
                    # m_CK is in between m_k and m_lag
                    a1, a2 = integrate_v_helper(model, a, bi, biprime, yi, m_lag, m_CK)
                    a3, a4 = integrate_v_helper(model, a, bi, biprime, yi, m_CK, m_k)
                    v_m += a1 + (1 - η) *  a3
                    bond_return_val += a2 + (1 - η) * a4
                end 
                (cur.m >= mD) && break  # we are done here 
                m_lag = m_k
            end
            default_proba = (survival(m, mD) + η * (cdf(m, mD) - cdf(m, m_CK)))
            v_m += vD_val * default_proba
            set_v_new_at!(a, bi, yi, v_m)
            set_bond_return_at!(a, bi, yi, bond_return_val)
            set_repay_at!(a, bi, yi, 1 - default_proba)
