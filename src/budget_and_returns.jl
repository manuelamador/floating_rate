...
function _get_c(::MaturingCoupon, bond::FloatingRateBond;  y, b, κ, q, b_prime) 
    λ = get_λ(bond)
    return y + q * (b_prime - (1 - λ) * b) - (κ + λ) * b
end 
...
_risk_free_price(::MaturingCoupon, ::FloatingRateBond; R) = one(R)
...
function _find_bond_return(::MaturingCoupon, bond::FloatingRateBond; q, κ) 
    λ = get_λ(bond)
    return (λ + κ + (1 - λ) * q)
end 
...
function _yield(::MaturingCoupon, bond::FloatingRateBond; q, κ)
    λ = get_λ(bond)
    return (λ + κ) / q - λ
end 